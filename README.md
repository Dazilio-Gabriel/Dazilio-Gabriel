## 👋 Olá, eu sou o Gabriel Dazilio

🎓 Estudante e desenvolvedor de software com foco em **Backend**,  
atuando principalmente com **Java** e **SQL**, e estudando constantemente **boas práticas**, **POO** e **lógica de programação**.

---

### 🧠 Sobre mim

- 💻 Desenvolvedor backend com foco em **Java**
- 🗄️ Experiência com **MySQL** e modelagem de dados
- 📚 Estudando arquitetura, boas práticas e código limpo
- 🛠️ Vivência com suporte técnico e análise de sistemas
- 🚀 Buscando evolução contínua na área de desenvolvimento

---

### 🛠️ Tecnologias

<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="40" alt="Java" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/intellij/intellij-original.svg" height="40" alt="IntelliJ" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="40" alt="MySQL" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="40" alt="JavaScript" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="40" alt="HTML5" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="40" alt="CSS3" />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-plain.svg" height="40" alt="Git" />
</div>

---

### 📌 Interesses atuais

- Backend com **Java**
- Banco de dados relacionais (**MySQL**)
- Estruturas de dados e lógica
- Boas práticas e código limpo
- Desenvolvimento web como apoio ao backend

---

### 🔗 Conecte-se comigo

<div align="left">
  <a href="https://www.linkedin.com/in/gabriel-dazilio-fanchiotti-4253a2265/" target="_blank">
    <img src="https://raw.githubusercontent.com/maurodesouza/profile-readme-generator/master/src/assets/icons/social/linkedin/default.svg" width="40" height="40" alt="LinkedIn" />
  </a>
  <a href="mailto:gabriel.fanchiotti@hotmail.com" target="_blank">
    <img src="https://raw.githubusercontent.com/maurodesouza/profile-readme-generator/master/src/assets/icons/social/microsoft-outlook/default.svg" width="40" height="40" alt="Email" />
  </a>
</div>

---
